...
def run_training(model_name: str, batch: int, args) -> object:
...
def main() -> None:
    ...
    # --- train with CUDA-OOM auto-fallback (16 -> 8) ---
    try:
        results = run_training(args.model, args.batch, args)
    except (RuntimeError, Exception) as exc:  # noqa: BLE001
        msg = str(exc).lower()
        if "out of memory" in msg and args.batch > 8:
            print("\n[!] CUDA out of memory at batch "
                  f"{args.batch}. Retrying at batch 8...\n")
            try:
                import torch
                torch.cuda.empty_cache()
            except Exception:
                pass
            results = run_training(args.model, 8, args)
        else:
